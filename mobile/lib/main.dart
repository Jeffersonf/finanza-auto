...
class _CarHomeState extends State<CarHome> {
  ...
  Widget _maintenance(double width) { final expense = filtered.where((e) => !e.fuel).toList()..sort((a, b) => b.amount.compareTo(a.amount)); final cards = [['Próxima revisão', '—', 'cadastre troca de óleo ou revisão'], ['Prazo por data', '—', 'sem referência no histórico'], ['Maior gasto', expense.isEmpty ? '—' : _money(expense.first.amount), expense.isEmpty ? 'sem despesas' : _category(expense.first.category)], ['Hodômetro atual', '${maxOdo.round()} km', 'maior leitura importada'], ['Gasto em despesas', _money(expenseTotal), '${expense.length} lançamentos no filtro']]; return Column(children: [GridView.count(crossAxisCount: width < 560 ? 1 : 3, childAspectRatio: width < 560 ? 4.8 : 2.3, crossAxisSpacing: 10, mainAxisSpacing: 10, shrinkWrap: true, physics: const NeverScrollableScrollPhysics(), children: cards.map((c) => Container(padding: const EdgeInsets.all(14), decoration: BoxDecoration(color: surface2, borderRadius: BorderRadius.circular(13)), child: Column(crossAxisAlignment: CrossAxisAlignment.start, mainAxisAlignment: MainAxisAlignment.center, children: [Text(c[0], style: const TextStyle(color: Color(0xff939aaa), fontSize: 10, fontWeight: FontWeight.w800)), const SizedBox(height: 6), Text(c[1], style: const TextStyle(fontSize: 19, fontWeight: FontWeight.w800)), Text(c[2], style: const TextStyle(color: Color(0xff747c8c), fontSize: 10))])).toList())]); }
  ...
  Widget _history() => filtered.isEmpty ? const Padding(padding: EdgeInsets.all(35), child: Center(child: Text('Nenhum registro neste filtro.', style: TextStyle(color: Color(0xff9298a7)))) : Column(children: filtered.take(30).map((e) => ListTile(contentPadding: const EdgeInsets.symmetric(vertical: 2, horizontal: 2), leading: CircleAvatar(backgroundColor: e.fuel ? const Color(0x195af5c8) : const Color(0x19f5705a), child: Icon(e.fuel ? Icons.local_gas_station_outlined : Icons.build_outlined, color: e.fuel ? aqua : const Color(0xffff9b8c), size: 19)), title: Text(e.fuel ? '${e.fuelType} · ${e.liters.toStringAsFixed(2)} L' : (e.title.isEmpty ? _category(e.category) : e.title), style: const TextStyle(fontSize: 13, fontWeight: FontWeight.w700)), subtitle: Text([_date(e.date), if (e.odometer > 0) '${e.odometer.round()} km', if (e.note.isNotEmpty) e.note].join('  ·  '), maxLines: 1, overflow: TextOverflow.ellipsis, style: const TextStyle(color: Color(0xff9298a7), fontSize: 11)), trailing: Row(mainAxisSize: MainAxisSize.min, children: [Text(_money(e.amount), style: const TextStyle(color: Color(0xfff5705a), fontWeight: FontWeight.w800, fontSize: 12)), IconButton(icon: const Icon(Icons.delete_outline, size: 18, color: Color(0xff697282)), onPressed: () => _delete(e))])).toList());
